...
* SQLite FTS5 baseline по `chunks`;
* скрипт `scripts/search_chunks.py`;
* top-k результати пошуку;
* provenance для кожного результату: файл, шлях, сторінка, document_id, chunk_id;
* metadata filters: document_type, content_category, chunk_strategy;
* baseline-тестові запити для оцінки якості пошуку.

Критерій приймання:

* пошук повертає релевантні chunks;
* видно файл і сторінку;
* можна пояснити, чому результат знайдено;
* є top-k результати;
* OCR-документи не ламають пошук;
* код не залежить від LLM, embeddings або Qdrant;
* перевірені тестові запити:
  ...
  * `геопросторова підтримка Збройних Сил України`.
