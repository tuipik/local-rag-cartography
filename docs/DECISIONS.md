...
Документи відкриваються через backend endpoints `/documents/{document_id}/view` і `/documents/{document_id}/download`.

Не передавати абсолютні локальні шляхи у frontend.
...
Для локального single-user режиму може бути доданий окремий endpoint `open-local`, який відкриває файл на сервері через `xdg-open`, але він не є основним механізмом для multi-user deployment.

Причини:

- абсолютні шляхи не мають сенсу на клієнтській машині;
- не треба світити файлову структуру сервера;
- backend може контролювати доступ;
- у майбутньому можна додати auth і audit.

---

### Source display and citation links

Рішення:

У UI показувати всі retrieved sources, які були передані моделі як контекст.

Inline citations виду `[1]`, `[2]` у відповіді робити клікабельними посиланнями на відповідні документи.

Якщо відповідь повідомляє, що інформацію не знайдено, список джерел все одно показувати, але з назвою "Переглянуті джерела", а не "Джерела" чи "Література".

Причини:

- користувач має бачити, які документи система переглянула;
- не всі знайдені джерела обов'язково цитуються в тексті відповіді;
- при no-answer сценарії джерела є не доказами відповіді, а контекстом перевірки;
- це підвищує прозорість retrieval.
